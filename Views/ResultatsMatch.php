...
<head>
    ...
    <link rel="stylesheet" href="../css/ResultatsMatch.css"/>
<!-----------------TITLE A COMPLETER----------------->
...
<!-----------------SECTION 1----------------->       
            <div class="football_player_content_section">
                ...
                <form action=<?php echo "../Back/InsertOrUpdateResults.php?update=".$update; ?> method="POST" id="">
<!-----------------SOUS SECTION 1.1-----------------> 
...
<!-----------------BUTS-----------------> 
                <div class="football_player_content_subsection">
                           ...
                           foreach($buts as $but)
                            {
                                ...
                                echo '<a href="../Back/deleteBut.php?idBut='.$but["IdBut"].'&IdMatch='.$_GET['idFeuille'].'" ><button>Supprimer</button></a></div>';
                            }   
                        }
                    ?>
                    ...
                    <div id="AjoutBut">
                        <form method="POST" action="../Back/AddBut.php" id="ajoutButForm">
...
<!-----------------CHANGEMENTS----------------->                  
                <div class="football_player_content_subsection">
                    ...
                    <?php
                        if(count($changements) > 0)
                        {
                            foreach($changements as $changement)
                            {
                                ...
                                echo '<a href="../Back/deleteChangement.php?idChangement='.$changement["IdRemplacement"].'&IdMatch='.$_GET['idFeuille'].'" ><button>Supprimer</button></a></div>';
                            }   
                        }
                    ?>
                    <!-- Puis un formulaire pour en rajouter un !-->
                    ...
                    <div id="AjoutChangement">
                        <form action="../Back/AddChangement.php" method="POST" id="ajoutChangementForm">
                            <input type="hidden" name="IdMatch" value=<?php echo $_GET['idFeuille'];?>>
...
<!-----------------CARTONS----------------->    
                <div class="football_player_content_subsection">
                    ...
                    <?php
                        if(count($cartons) > 0)
                        {
                            foreach($cartons as $carton)
                            {
                                ...
                                echo '<a href="../Back/deleteCarton.php?idCarton='.$carton["IdCarton"].'&IdMatch='.$_GET['idFeuille'].'" ><button>Supprimer</button></a></div>';
                            }   
                        }
                    ?>
                    <!-- Puis un formulaire pour en rajouter un !-->
                    ...
                    <div id="AjoutCarton">
                        <form action="../Back/AddCarton.php" method="POST" id="ajoutCartonForm">
                            <input type="hidden" name="IdMatch" value=<?php echo $_GET['idFeuille'];?>>
...
<!--Submit button-->
        <form action="../Back/ValiderResultats.php" method="POST" id="validerResultatsForm">
        
        <input type="hidden" name="IdMatch" value=<?php echo $_GET['idFeuille'];?>>
        <input type="submit" value="Valider la feuille de match" class="submit_button">
        </form> 
    </div>
    <script src="../jsScripts/resultatsMatch.js"></script>
</body>
